...
public class EnvironmentAnalyzer implements Serializable {
    ...
    final static private long encodedVersion = 2___00001_017___2025_12_30__01L;
    //-----------------------------------------#---^^^^^-^^^---^^^^-^^-^^--^^
    ...
    /**
     * Determine amount of free memory in the Java Virtual Machine,
     * 
    ...
    /**
     * Determine total amount of memory in the Java virtual machine.
     * 
    ...
    /**
     * Determine maximum amount of memory that the Java virtual machine will attempt to use.
     * If there is no inherent limit then the value java.lang.Long.MAX_VALUE will be returned.
     * 
     * @return  maximum amount of available memory
     */
    ...
    /**
     * Determine Java version (the old way)
     * 
     * @return java version
     */
    public static String determineJavaVersionTheOldWay(){
    ...
    // See https://docs.oracle.com/cd/E15289_01/JRCLR/sysprop.htm  for "property keys"
    // Works fine for:
    //  Temurin-25.0.1+8-LTS
    /**
     * Determine Java version. <br />
     * Note: It's more checking for wanted (Eclipse-)Temurin / (Adopt-)OpenJDK / Adoptium - "Java".
     * It might NOT be suited for other java vendor versions.
     * 
     * @return java version
     */
    public static String determineJavaVersion(){
        final String javaRuntimeEnvironmentVendor = System.getProperty( "java.vendor" );
        final String javaVirtualMachineImplementationVendor = System.getProperty( "java.vm.vendor" );
        if( null==javaRuntimeEnvironmentVendor )  throw new RuntimeException( "Uuuupppss - java.vendor delivers null" );
        if( null==javaVirtualMachineImplementationVendor )  throw new RuntimeException( "Uuuupppss - java.vm.vendor delivers null" );
        // check for Eclipse Adoptium "=" Eclipse Adoptium
        if(   ! Objects.equals( javaRuntimeEnvironmentVendor, javaVirtualMachineImplementationVendor )
            &&  -1 == javaRuntimeEnvironmentVendor.indexOf( javaVirtualMachineImplementationVendor )
            &&  -1 == javaVirtualMachineImplementationVendor.indexOf( javaRuntimeEnvironmentVendor )
        ){
            throw new RuntimeException( String.format(
                "UNEXPECTED values : RE-> %s  <->  %s <-VM", 
                javaRuntimeEnvironmentVendor,
                javaVirtualMachineImplementationVendor
            ));
        }//if            
        //\=> VM≙RE
        //
        final String javaVersion = System.getProperty( "java.version" );
        final String javaRuntimeEnvironmentVersion = System.getProperty( "java.vendor.version" );
        final String javaVirtualMachineImplementationVersion = System.getProperty( "java.vm.version" );
        if( null==javaVersion )  throw new RuntimeException( "Uuuupppss - java.version delivers null" );
        if( null==javaRuntimeEnvironmentVersion )  throw new RuntimeException( "Uuuupppss - java.vendor.version delivers null" );
        if( null==javaVirtualMachineImplementationVersion )  throw new RuntimeException( "Uuuupppss - java.vm.version delivers null" );
        //
        // NOTE: 25.0.1/Temurin-25.0.1+8/25.0.1+8-LTS
        final int posi = javaRuntimeEnvironmentVersion.indexOf( javaVersion );                      // TODO better name
        boolean problem = ( -1 == posi );
        if( ! problem ){
            final String rest = javaRuntimeEnvironmentVersion.substring( posi );
          //final int posi2 = javaVirtualMachineImplementationVersion.indexOf( javaVersion );       // TODO better name
            problem = problem || ( 0 != javaVirtualMachineImplementationVersion.indexOf( rest ) );
        }//if
        final StringBuilder version = new StringBuilder();
        version.append( javaRuntimeEnvironmentVersion.substring( 0, posi ) );
        version.append( javaVirtualMachineImplementationVersion );
        final String theVersion = version.toString();
        final String javaVersionTOW = determineJavaVersionTheOldWay();
        problem = problem || ( -1 == theVersion.indexOf( javaVersionTOW ));
        if( problem )  throw new RuntimeException( String.format(
            "UNEXPECTED values : %s <-> %s <-> %s <-> %s",
            javaVersionTOW,
            javaVersion,
            javaRuntimeEnvironmentVersion,
            javaVirtualMachineImplementationVersion
        ));
        //
        return String.format(
            "\"%s\" @ %s from \"%s\" at \"%s\"    🙂VM≙RE🙂",
            theVersion,
    ...
    //###
    //###
    //###   JUnit related  -  remove if not required
    //###
    ...
    //###
    //###   Last Topic:
    //###   Determine JUnit related versions
    //### 
    //###   JUnit is hell - how do you get its version easily and timelessly?
    //###   Note -> see https://stackoverflow.com/questions/59377304/accessing-junit-version-during-runtime
    //###   With JUnit 4, it took forever to support access to the corresponding version at runtime:
    //###   \-> [JUnit4]:   "junit.runner.Version.id()"
    //###
    //###   JUnit5, with its 3 modules, is still waiting for a counterpart.
    //###   The following workaround/code is written in "slow motion" for maximum understanding.
    //###   Furthermore, deprecated code must be used to have at least some way.
    //###
    //###
    //###   250716:
    //###   Excessive checks for null as result of mysterious effects (NullPointerException) in the past.
    //###   The code is almost performance-hostile but hopefully as easy to maintain as possible.
